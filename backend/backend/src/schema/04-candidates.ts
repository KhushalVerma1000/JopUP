...
export const candidate = pgTable("candidate", {
  ...
  // Nullable — unlike every other created_by in this schema, candidates can
  // also be created with NO staff actor at all: the public job-portal
  // "guest apply" flow (job-portal.service.js's submitApplication) creates
  // a candidate from an anonymous applicant's own submission, with no
  // logged-in user to attribute it to. Confirmed via the e2e test script
  // against a live DB — that flow 500'd on a NOT NULL violation until this
  // was made nullable, matching updatedBy just below (already nullable for
  // the same reason).
