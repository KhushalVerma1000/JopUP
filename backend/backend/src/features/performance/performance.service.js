...
class PerformanceService {
  ...
  async createKpiEntry(data, userId) {
    // periodDate is optional in createKpiEntrySchema, but the DB column
    // (kpi_entry.period_date) is NOT NULL with no database-level default —
    // so omitting it always 500'd with a NOT NULL violation, confirmed via
    // the e2e test script against a live DB. Defaulting to today here
    // actually honors what the schema already promises callers.
    const [entry] = await db.insert(schema.kpiEntry).values({
      ...data,
      periodDate: data.periodDate || new Date().toISOString().slice(0, 10),
      recordedBy: userId
    }).returning();
    return entry;
  }
